...
1) 원자성(Atomicity)
트랜잭션은 연산들을 하나의 작업으로 취급하여 전부 실행하든지 전혀 실행하지 않아야지 일부만 실행해서는 안된다. 

2) 일관성(Consistency)
트랜잭션이 성공적으로 실행되면 데이터베이스 상태는 모순되지 않고 일관된 상태가 되어야지 바뀌면 안된다.

3) 격리성(Isolation)
트랜잭션 실행 도중의 연산 결과는 다른 트랜잭션에서 접근할 수 없다.

4) 영속성(Durability)
트랜잭션이 성공적으로 완료되면 그 결과는 영속적이어야지 어떠한 천재지변이 발생하더라도 소실되면 안된다.
